...
from new_load_dataset import create_train_test_split
from util import (
    GINSWDSingleModel,
...
def get_args():
    ...
    parser.add_argument('--id_dataset', type=str, default='BZR',
                        choices=['BZR', 'PTC-MR', 'AIDS', 'ENZYMES', 'IMDB-MULTI', 'Tox21', 'FreeSolv',
                                 'BBPB', 'ClinTox', 'Esol', ],
                        help='ID dataset name (known classes for training)')
    parser.add_argument('--ood_dataset', type=str, default='COX2',
    ...
    parser.add_argument('--num_atom_supp', type=int, default=24,
    ...
    parser.add_argument('--w_atom', type=float, default=0.5660027935623939,
                        help='Atom distance loss weight')
    parser.add_argument('--w_pos', type=float, default=1.5763036382481996,
                        help='Positive sample distance loss weight')
    parser.add_argument('--w_neg', type=float, default=1.2698722391046098,
...
def evaluate_energy_based_auto(model, test_data, num_known, device, threshold_method='gaussian_tail', n_std=2.0):
    ...
    return auc, binary_recall, binary_precision, binary_f1, known_accuracy, all_energy_values, all_labels, auto_threshold
...
def train_and_evaluate(args):
    ...
    auc, binary_recall, binary_precision, binary_f1, known_accuracy, all_energy_values, all_labels, auto_threshold = evaluate_energy_based_auto(
    ...
    return model, auc, binary_recall, binary_precision, binary_f1, known_accuracy, all_energy_values, all_labels, auto_threshold
...
def main():
    ...
    pre_list = []
    for i in range(5):
        model, auc, binary_recall, binary_precision, binary_f1, known_accuracy, all_energy_values, all_labels, auto_threshold = train_and_evaluate(
            args)
        auc_list.append(auc)
        acc_list.append(known_accuracy)
        f1_list.append(binary_f1)
        rec_list.append(binary_recall)
        pre_list.append(binary_precision)

    print(np.mean(auc_list), np.std(auc_list))
    print(np.mean(acc_list), np.std(acc_list))
    print(np.mean(f1_list), np.std(f1_list))
    print(np.mean(rec_list), np.std(rec_list))
    print(np.mean(pre_list), np.std(pre_list))
